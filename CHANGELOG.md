...
## [Unreleased]
### Changed
- Completions are now generated by indexing the open Drupal codebase (`*.api.php`, `*.services.yml`, `Element/*.php`) instead of shipping static snippets, so they match the installed core version and include contrib and custom code.
- Files are watched; new, edited, and deleted files (and directories added by composer) update completions without a reload.
- Hooks: OOP `#[Hook]` snippets in `src/Hook/`, procedural elsewhere; placeholders such as `ENTITY_TYPE` are tab stops; `@deprecated` messages are written into the docblock.
- Services: `service:` prefix; class resolved through aliases, `parent:`, and class-keyed IDs; `assert()` uses the fully qualified class; deprecation shown in the docs and as a `// @deprecated` line.
- Elements: `element:` prefix; attributes and legacy annotations both supported; documented properties become tab stops.
- Deprecated hooks and services are struck through in the completion list.
- Minimum VS Code version is 1.97.
### Added
- **Drupal Smart Snippets: Reindex** command.
- Unit tests, a Drupal core + webform compatibility suite, and CI that packages a `.vsix` for every pull request.
- Developer docs in `docs/`.
### Removed
- Static snippet files and the scraper pipeline.
- Support for Drupal 8 and 9 code that has no `core/lib/Drupal.php` web root.

## [2.4.0]
- No changelog recorded for this release.
